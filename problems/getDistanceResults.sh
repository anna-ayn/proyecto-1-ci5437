...
# Obtenemos las distancias de A* y las mostramos en consola (Recordamos eliminar la "," de las distancias)
echo "A*"
head -n $(($lineaIDA-1)) $1 | grep "Estado objetivo encontrado con distancia" | cut -d" " -f6 | tr -d ","
...
# Obtenemos las distancias de IDA* y las mostramos en consola
echo "IDA*"
tail -n $(($lineas-$lineaIDA-1)) $1 | grep "Estado objetivo encontrado con distancia" | cut -d" " -f6 | tr -d ","
